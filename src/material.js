...
export class Sprite {
    ...
    setup() {
        ...
        this.uv_x = 1//this.size.x / this.image.width;
        this.uv_y = 1//this.size.y / this.image.height;
    ...
    render(position, /*frames = { x: 0, y: 0 },*/ options) {
        ...
        if (this.isLoaded) {
            ...
            // let o_trans = new M3x3()
            let o_scale = new M3x3().transition(position.x, position.y);
            ...
            for (let option in options) {
                this.material.set(option, ...options[option])
                //this.material.set.apply(this.material, [option].concat(options[option]));
                if (option == "scalex") { o_scale = o_scale.scale(options.scalex, 1.0); }
                if (option == "scaley") { o_scale = o_scale.scale(1.0, options.scaley); }
            }
            ...
            this.material.set("u_scale", o_scale.getFloatArray());
            //  this.material.set("u_transition", o_trans.getFloatArray())
...
export class BackBuffer {
    constructor(gl, options, VS = BackBuffer.VS, FS = BackBuffer.FS) {
        this.gl = gl;
        this.material = new Material(
            this.gl,
            VS, FS
        );
        this.size = new Vector(512, 512);
...
export class MapBackBuffer extends BackBuffer {
    constructor(gl, options) {
        super(gl, options, MapBackBuffer.VS, BackBuffer.FS)
    }
    render(options = {}) {
        this.scaleMat = new M3x3().scale(options.get("scalex", 1), options.get("scaley", 1))
        this.transMat = new M3x3().transition(options.get("x", 0), options.get("y", 0))
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER, this.geo_buff);
        this.material.set("a_position");
        this.material.set("u_transition", this.transMat.getFloatArray())
        this.material.set("u_scale", this.scaleMat.getFloatArray())
...
varying vec2 v_texCoord;
void main(){
    gl_Position = vec4(u_scale*u_transition*vec3(a_position, 1), 1);
    v_texCoord = a_texCoord;
}
`
